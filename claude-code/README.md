...
Agent Skills と Subagents の標準的な配置・形式です。

- Agent Skills: https://code.claude.com/docs/en/skills
- Subagents: https://code.claude.com/docs/en/sub-agents

`reviewer.md` は `tools: Read` だけを割り当てています。Claude Code の Subagents
機能は frontmatter の `tools` で使用可能なツールを制限できるため、この
subagent には構造的にファイル書き込みができません。

`tools/logs_insights.py` と `tools/span_filter.py` はこのディレクトリの外
(リポジトリ ルートの `tools/`) にあります。3 つの subagent はいずれも
`../tools/...` という相対パスで参照する前提になっています。そのため、Claude
Code は必ずこの `claude-code/` ディレクトリを起点 (cwd) にして起動してください。
...
3. このディレクトリに移動して Claude Code を起動します。
...
4. Skill を呼び出します。Claude Code は `.claude/skills/` を自動的に走査するため、
   調査内容を伝えるだけで Skill が起動します。
...
Claude Code は Amazon Bedrock 上のモデルを使う設定を公式にサポートしています。

出典: https://code.claude.com/docs/en/amazon-bedrock (取得日 2026-09-24)

- 環境変数 `CLAUDE_CODE_USE_BEDROCK=1` と `AWS_REGION` を設定します。モデルを
  ...
  のようなクロスリージョン推論プロファイル ID) を指定します。プロファイルの
  接頭辞 (`us.` / `eu.` / `apac.` / `us-gov.`) はリージョンに応じて解決されます。
- 必要な IAM 権限は `bedrock:InvokeModel`、`bedrock:InvokeModelWithResponseStream`、
  `bedrock:ListInferenceProfiles`、`bedrock:GetInferenceProfile` です。
- 認証方法は次の 5 種類がドキュメントに記載されています。AWS CLI プロファイル、
...
> [!NOTE]
> 上記は 2026-09-24 に公式ドキュメントで確認した内容です。モデル ID や
> IAM 権限の名称は変わる可能性があるため、実行前に上記の公式ドキュメントで
> 最新の内容を確認してください。

## 前提条件

- Claude Code (Amazon Bedrock 対応バージョン) がインストールされている必要が
  あります。
- Amazon Bedrock でモデルを呼び出すための AWS 認証情報と IAM 権限
  (上記「Amazon Bedrock の設定」を参照) が設定されている必要があります。
